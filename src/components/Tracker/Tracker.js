...
const Tracker = ({item: {id, name, time, isTimerActive, snapShot}}) => {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      setTracker({seconds: Math.ceil(now - snapShotTik + time.seconds)})
    }
  }, [])
  
  useEffect(() => {
    let timerId;
    dispatch(updateTracker({time: tracker, isTimerActive: isActive, snapShot: snapShotTik}))
    if (isActive) {
      timerId = setInterval(() => {
        setTracker(prev => { 
  ...
  }, [tracker, isActive, snapShotTik, dispatch]);
